...
import threading
import time
import subprocess  # 添加subprocess模块用于启动外部程序
...
from PySide2.QtGui import (QImage, QPixmap, QPainter, QPen, QColor, QFont, QIcon,
                         QTransform, QKeySequence, QPalette, QBrush, QPainterPath,
                         QRegion, QCursor, QFontMetrics, QMouseEvent)
from PySide2.QtCore import Qt, QTimer, QPointF, QEvent, QSize, QRect, QPoint, QCoreApplication, Signal, QThread
...
class VideoAnnotationApp(QMainWindow):
    def __init__(self):
        ...
        self.zoom_factor = 1.0  # 默认缩放比例改为1:1
        ...
        self.last_touch_distance = 0  # 添加用于双指缩放的变量
    ...
    def init_ui(self):
        ...
        perspective_action.triggered.connect(self.open_perspective_correction_exe)
    ...
    def open_perspective_correction_exe(self):
        """打开梯形校正程序（correction.exe）"""
        # 检查当前目录下是否存在correction.exe
        exe_path = "correction.exe"
        if os.path.exists(exe_path):
            try:
                ...
                self.status_bar.showMessage("已启动梯形校正程序")
            except Exception as e:
                QMessageBox.critical(self, "错误", f"启动校正程序失败: {str(e)}")
        else:
            QMessageBox.warning(self, "警告", "未找到校正程序: correction.exe")
    ...
    def handle_touch_begin(self, event):
        ...
        # 双指触控：开始缩放
        elif len(self.last_touch_points) == 2:
            # 计算初始距离
            ids = list(self.last_touch_points.keys())
            p1 = self.last_touch_points[ids[0]]
            p2 = self.last_touch_points[ids[1]]
            self.last_touch_distance = (p1 - p2).manhattanLength()
            return True
    ...
    def handle_touch_update(self, event):
        ...
        # 双指缩放
            ...
            p1 = touch_points[ids[0]]
            p2 = touch_points[ids[1]]
            current_distance = (p1 - p2).manhattanLength()
            
            if self.last_touch_distance > 0:
                # 计算缩放比例
                scale_factor = current_distance / self.last_touch_distance
    ...
    def handle_touch_end(self, event):
        ...
        self.touch_drawing = False
        self.last_touch_points = {}
        self.last_touch_distance = 0  # 重置触摸距离
